...
from .models import App, Module, Challenge, Level, LevelResponses, Categories, Skill, SkillResponses, Section, Topic, Lession, Question,ContactForm
...
def get_challenges_labels(request, user, app_id, module_id):
    ...
    for ch in challenge:
        ...
        for level in Level.objects.filter(challenge_id=ch.id).order_by("order_of_display"):
            ...
            nlr_first = nlr.first()
            topic_names = [topic.name for topic in level.topics.all()]
            nl = {
                'id': level.id,
...
@api_view(['POST'])
@isAuth
def addconatctus(request,user):
    # data=request.data
    print(request.POST)
    print(user)
    conatctusdata = ContactForm(
    ...
    return Response({'message': 'Contact Us is added successfully'})


@api_view(['POST'])
@isAuth
def updateconatctus(request,id):
    # print(request.data,'ssssssssss')
    # data=request.data
    print(id)
    conatctusdata = ContactForm.objects.get(id=int(id))
    conatctusdata.replay = request.POST.get['replay']
    conatctusdata.save()
    
    return Response({'message': 'Contact Us is Update successfully'})


@api_view(['GET'])
@isAuth
def viewconatctus(request):
    records = ContactForm.objects.all()
    return Response([{
        "id":record.id,
        'name': record.name,
...
@api_view(['GET'])
@isAuth
def viewconatctusone(request,id,user):
    records = ContactForm.objects.filter(id=id)
    return Response([{
        "id":record.id,
        'name': record.name,
        'message': record.message,
